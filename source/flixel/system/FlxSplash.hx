...
class FlxSplash extends FlxState {
	...
	override public function create():Void {
		...
		#if FLX_SOUND_SYSTEM
		if (!muted) {
			soudn = FlxG.sound.load(FlxAssets.getSound("flixel/sounds/flixel"));
		}
		#end
	}
	...
	function timerCallback(Timer:FlxTimer):Void {
		...
		if (_curPart == 5) {
			...
			FlxTween.tween(_sprite, {alpha: 0}, 2.0, {startDelay: 1, ease: FlxEase.quadOut, onComplete: (_) -> complete()});
			FlxTween.tween(_text, {alpha: 0}, 2.0, {startDelay: 1, ease: FlxEase.quadOut});
		}

		#if FLX_SOUND_SYSTEM
		if (!muted && soudn != null && _curPart == 1) {
			soudn.play();
		}
		#end
	}
